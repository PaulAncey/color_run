...
public class ParticipationServlet extends HttpServlet {
    ...
    private void listUserParticipations(HttpServletRequest request, HttpServletResponse response)
        ...
        // Récupérer les participations de l'utilisateur avec les informations des
        // courses
        try {
        ...
        try {
            ...
            if (participationService.trouverParUtilisateurEtCourse(userId, courseId).isPresent()) {
                response.sendRedirect(request.getContextPath() + "/courses/" + courseId
                        + "?error=Vous êtes déjà inscrit à cette course");
                return;
            }
            ...
            if (participationService.courseEstComplete(courseId)) {
                response.sendRedirect(
                        request.getContextPath() + "/courses/" + courseId + "?error=Cette course est complète");
                return;
            }
            ...
            response.sendRedirect(request.getContextPath() + "/courses/" + courseId
                    + "?success=Inscription réussie ! Vous pouvez maintenant télécharger votre dossard.");
    ...
    private void cancelRegistration(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!participationService.trouverParUtilisateurEtCourse(userId, courseId).isPresent()) {
                response.sendRedirect(
                        request.getContextPath() + "/participations?error=Vous n'êtes pas inscrit à cette course");
                return;
            }
            ...
            response.sendRedirect(
                    request.getContextPath() + "/participations?error=Erreur lors de l'annulation : " + e.getMessage());
        }
    }
    ...
    private void downloadBib(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.setHeader("Content-Disposition",
                    "attachment; filename=\"dossard_" + participation.getNumeroDossard() + ".pdf\"");
